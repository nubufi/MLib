...
class DataProcessor:
    ...
    def grab_col_names(self, summary=False):
        # cat_cols, cat_but_car
        cat_th = self.config.categorical_threshold
        car_th = self.config.cardinal_threshold
        all_columns = self.data_frame.columns
        cat_cols = [col for col in all_columns if self.data_frame[col].dtypes == "O"]
        num_but_cat = [
            col
            for col in all_columns
            if self.data_frame[col].nunique() < cat_th
               and self.data_frame[col].dtypes != "O"
        ]
        cat_but_car = [
            col
            for col in all_columns
            if self.data_frame[col].nunique() > car_th
               and self.data_frame[col].dtypes == "O"
        ]
        ...
        # num_cols
        num_cols = [col for col in all_columns if self.data_frame[col].dtypes != "O"]
        num_cols = [col for col in num_cols if col not in num_but_cat]

        if summary:
            print(f"Observations: {self.data_frame.shape[0]}")
            print(f"Variables: {self.data_frame.shape[1]}")
    ...
    def process_data(self):
        cat_cols, num_cols, cat_but_car = self.grab_col_names()
    ...
    def process_as_previous(self, df):
        cat_cols, num_cols, _ = self.grab_col_names()

...
class Regression:
    def __init__(
            self, x: pd.DataFrame, y: pd.DataFrame, config: RegressionModel
    ):
        self.X = x
        self.y = y
        self.config = config
    ...
    def get_base_models(self):
        print("Base Models....")
        regressors = [("RF", RandomForestRegressor(), self.config.rf_params),
        ...
        print(f"Best {self.config.number_of_models} models: {[name for name, _, _ in best_n_models]}")
    ...
    def create_model(self) -> VotingRegressor:
        ...
        cv_results = cross_validate(
            self.model, self.X, self.y, cv=self.config.number_of_folds, scoring=self.config.scoring,
            error_score="raise")
